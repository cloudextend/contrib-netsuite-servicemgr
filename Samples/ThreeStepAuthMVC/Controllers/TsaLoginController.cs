...
namespace ThreeStepAuthMVC.Controllers
{
    ...
    public class TsaLoginController : Controller
    {
        ...
        public TsaLoginController(IAccessTokenService accessTokenService,
        ...
        public async Task<ActionResult<RequestTokenResponse>> 
            RequestToken(string account)
        {
            var response = await _requestTokenService.GetRequestToken(account);
            return (response.Error == null) ? Ok(response) : 
                StatusCode((int)response.Error.StatusCode, response);
        }

        [HttpGet("authorized-token")]
        public async Task<ActionResult<AccessTokenResponse>> AuthorizeToken(
            string account, string requestToken, string tokenSecret, 
            string verifier)
        {
            var response = await _accessTokenService.GetAccessToken(
        ...
        public async Task<ActionResult> RevokeToken(string account, 
            string token, string tokenSecret)
        {
            var response = await _revokeTokenService.RevokeToken(account,
                token, tokenSecret);
            return response.Error == null ? Ok(response) : 
                StatusCode((int)response.Error.StatusCode, response);
        }
